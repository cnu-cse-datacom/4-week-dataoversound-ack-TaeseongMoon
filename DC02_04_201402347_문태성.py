...
def encode(bits_stream):
    frequences = []
    frequences.append(HANDSHAKE_START_HZ)
    for i in bits_stream:
        hz = START_HZ+i*STEP_HZ
        frequences.append(hz)
    frequences.append(HANDSHAKE_END_HZ)
    p = pyaudio.PyAudio()
    ...
    for f in frequences:
        if n == 0:
            print("Start_Hz")
        elif n == len(bits_stream)+1:
            print("End_Hz")
        else:
            print(bits_stream[n-1])
        n += 1
        print("frequence:", f)
        sample = np.arange(44100*0.5)
        samples = (np.sin(2*np.pi*sample*f/44100)).astype(np.float32)
    ...
    while True:
        ...
        if in_packet and match(dom, HANDSHAKE_END_HZ):
            ...
            try:
                ...
                byte_stream = byte_stream.decode("utf-8") # askii decode
                print(byte_stream)
                ID = []
                checkID=True
                for i in range(len(my_ID)):
                    if my_ID[i] == byte_stream[i]:
                ...
                if checkID:
                    ...
                    bytes_stream = bytes_stream[:0] + bytes_stream[9:]
                    display(byte_stream)
                    display("")
                    bits_stream = []
                    for i in range(len(bytes_stream)):
                        bits_stream.append(bytes_stream[i]>>4)
                        bits_stream.append(bytes_stream[i]&15)
                    print(bits_stream)
                    encode(bits_stream)
